...
import { MessageCircle, Users, User, Mountain, Search, ArrowLeft } from 'lucide-react'
...
export function ChatLayout({ rooms, currentUser }: ChatLayoutProps) {
  ...
  return (
    <div className="h-[calc(100dvh-64px)] flex bg-background">
      {/* Sidebar */}
      <div className={`w-full md:w-96 md:min-w-[384px] border-r border-border flex flex-col bg-background ${selectedRoomId ? 'hidden md:flex' : 'flex'}`}>
        {/* Sidebar header */}
      ...
      {/* Right panel — empty state on desktop when no room selected */}
      {!selectedRoomId && (
